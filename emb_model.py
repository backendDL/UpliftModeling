...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.drop(
            self.relu(
                self.layers_agg(x.view(-1, x.shape[-1])).view(x.shape[0], x.shape[1])
            )
        )
        x = self.end(x)
        return x
...
class Emb(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers=1, drop_prob=0.2):
        super().__init__()
        self.input_dim = input_dim  # vector_len including one-hot
        self.hidden_dim = hidden_dim
        self.output_dim = output_dim  #
        self.n_layers = n_layers  # 아마 1
    ...
    def forward(self, x):
        # x가 시퀀스 전체 (batch, num_log, log_dim)
        h = torch.zeros(self.n_layers, x.size(0), self.hidden_dim).to(self.device)
        for i in range(x.size(1)):
            output, h = self.gru(x[:, i, :].unsqueeze(1), h)
        return self.fc(output)


if __name__ == "__main__":
    model = Emb(12, 128, 64).to(torch.device("cuda"))
    data = torch.randn(64, 50, 10).to(
        torch.device("cuda")
    )  # 배치 64, 50일치 데이터, 한 vector가 10차원
    output = model(data)
    print(output.shape) 
